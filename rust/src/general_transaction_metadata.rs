...
pub unsafe extern "C" fn cardano_transaction_metadatum_new_bytes(
    bytes: CData,
    result: &mut TransactionMetadatum,
    error: &mut CError,
) -> bool {
...
pub unsafe extern "C" fn cardano_transaction_metadatum_decode_arbitrary_bytes_from_metadatum(
    transaction_metadatum: TransactionMetadatum,
    result: &mut CData,
    error: &mut CError,
) -> bool {
    handle_exception_result(|| {
...
pub unsafe extern "C" fn cardano_transaction_metadatum_encode_json_str_to_metadatum(
    ...
    handle_exception_result(|| {
        json.unowned()
            .and_then(|json| {
                encode_json_str_to_metadatum(json.to_string(), schema.into()).into_result()
            })
            .and_then(|transaction_metadatum| transaction_metadatum.try_into())
    })
    .response(result, error)
            ...
            (0..tm_labels.len())
                ...
                .map(|tm_label| {
                    general_transaction_metadata
                        .get(&tm_label)
                        .ok_or(
                            "Cannot get TransactionMetadatum by TransactionMetadatumLabel".into(),
                        )
