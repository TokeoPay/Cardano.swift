        ...
        tb.inputs
            ...
            .map(
                |((((((mut new_tb, certs), wls), update), mint), collateral), required_signers)| {
                    ...
                    required_signers
                        .map(|required_signers| new_tb.set_required_signers(&required_signers));
...
impl TryFrom<RTransactionBody> for TransactionBody {
    ...
    fn try_from(tb: RTransactionBody) -> Result<Self> {
        tb.inputs()
            ...
            .map(
                |(
                    ((((((inputs, outputs), certs), withdrawals), update), mint), collateral),
                    r_signers,
                )| {
                    Self {
                        inputs,
                        ...
                        ttl: tb
                            .ttl_bignum()
                            .map(|bn| COption::Some(Into::<u64>::into(bn)))
                            .unwrap_or(COption::None),
                        certs: certs.into(),
                        withdrawals: withdrawals.into(),
                        update: update.into(),
                        auxiliary_data_hash: tb
                            .auxiliary_data_hash()
                            .map(|hash| hash.into())
                            .into(),
                        validity_start_interval: tb
