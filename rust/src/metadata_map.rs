...
impl Free for MetadataMap {
    unsafe fn free(&mut self) {
        ...
        let mut vals =
            Vec::from_raw_parts(self.cptr as *mut MetadataMapKeyValue, self.len, self.len);
    ...
    unsafe fn unowned(&self) -> Result<&[MetadataMapKeyValue]> {
        if self.cptr.is_null() {
            ...
            Ok(std::slice::from_raw_parts(
                self.cptr as *const MetadataMapKeyValue,
                self.len,
            ))
        }
    }
...
pub unsafe extern "C" fn cardano_metadata_map_from_array(
    array: *const MetadataMapKeyValue,
    len: usize,
    result: &mut MetadataMap,
    error: &mut CError,
) -> bool {
    handle_exception(|| MetadataMap {
