...
use cardano_serialization_lib::{
    address::Address as RAddress, crypto::PrivateKey as RPrivateKey,
...
pub unsafe extern "C" fn cardano_private_key_sign_data(
    ...
    handle_exception_result(|| {
        let res: std::prelude::v1::Result<std::prelude::v1::Result<DataSignature, CError>, CError> =
            TryInto::<RAddress>::try_into(address)
                .zip(TryInto::<RPrivateKey>::try_into(private_key))
                .map(|(addr, sk)| {
                    let payload: &[u8] = message.unowned().unwrap();

                    let mut key =
                        ms::COSEKey::new(&ms::Label::new_int(&ms::utils::Int::new_i32(0)));
